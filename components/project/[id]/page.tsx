...
export default async function ProjectDetailPage({
    ...
    return (
      <div className="min-h-screen bg-[#09090b] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/dashboard" className="text-[#10b981] text-sm mt-4 inline-block">
            ← Back to Dashboard
          </Link>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="bg-[#09090b] min-h-screen dot-grid">

        {/* Navbar */}
        <header className="fixed top-0 w-full h-[60px] backdrop-blur-xl border-b flex justify-between items-center px-6 z-50"
          style={{background:'rgba(14,19,34,0.6)', borderColor:'rgba(39,39,42,0.15)'}}>
          <Link href="/dashboard"
            className="flex items-center gap-2 hover:text-[#10b981] transition-colors"
            style={{color:'#d4d4d8', fontFamily:'DM Mono', fontSize:'13px'}}>
            <span className="material-symbols-outlined" style={{fontSize:'18px'}}>arrow_back</span>
            Back to Feed
          </Link>
          <div style={{fontFamily:'Inter', fontSize:'20px', fontWeight:900, letterSpacing:'-0.05em', color:'#10b981'}}>
            PROJECT_HUB
          </div>
        ...
        <main className="pt-[80px] pb-16 px-4 max-w-5xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span style={{
                ...
                background: project.status === 'open' ? 'rgba(16,185,129,0.1)' : 'rgba(39,39,42,0.2)',
                color: project.status === 'open' ? '#10b981' : '#71717a'
              }}>
                {project.status}
              </span>
            ...
            <h1 style={{fontFamily:'Inter', fontSize:'36px', fontWeight:800, lineHeight:1.1, marginBottom:'16px'}}>
              {project.title}
            </h1>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {project.required_skills?.map(skill => (
                <span key={skill} style={{
                  ...
                  background:'#18181b', fontSize:'12px',
                  fontFamily:'DM Mono', color:'#d4d4d8'
                }}>
                  {skill}
                </span>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {session ? (
                <div className="rounded-md p-6 glass-card"
                  style={{border:'1px solid rgba(39,39,42,0.15)'}}>
                  <h2 style={{fontFamily:'Inter', fontSize:'18px', fontWeight:700, marginBottom:'16px'}}>
                    About This Project
                  </h2>
                  <p style={{color:'#d4d4d8', fontSize:'15px', lineHeight:1.8, whiteSpace:'pre-wrap'}}>
                    {project.description}
                  </p>
                </div>
              ) : (
                ...
                <div className="rounded-md p-6 relative overflow-hidden"
                  style={{border:'1px solid rgba(39,39,42,0.15)', background:'rgba(24,24,27,0.4)'}}>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center"
                    ...
                    <span className="material-symbols-outlined" style={{fontSize:'32px', color:'#10b981', marginBottom:'12px'}}>lock</span>
                    <p style={{fontFamily:'DM Mono', fontSize:'13px', color:'#10b981', marginBottom:'16px'}}>
                      Sign in to view full details
                    </p>
                    <Link href="/login">
                      <button className="px-6 py-2 rounded-sm text-sm font-bold"
                ...
                <div className="rounded-md p-6"
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <span className="material-symbols-outlined"
                      style={{fontSize:'20px', color: isTeamMember || isOwner ? '#6bd8cb' : '#27272a'}}>
                      {isTeamMember || isOwner ? 'lock_open' : 'lock'}
                    </span>
                    <h2 style={{fontFamily:'Inter', fontSize:'18px', fontWeight:700}}>
                      Team Vault
                    </h2>
                    ...
                    <div className="space-y-4">
                      {project.github_repo && (
                        <a href={project.github_repo} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-3 rounded-md hover:bg-[#18181b] transition-all"
                          style={{border:'1px solid rgba(39,39,42,0.2)'}}>
                          <span className="material-symbols-outlined" style={{color:'#10b981'}}>code</span>
                          <div>
                            <div style={{fontFamily:'DM Mono', fontSize:'12px', color:'#10b981'}}>GitHub Repository</div>
                            <div style={{fontSize:'11px', color:'rgba(194,198,214,0.5)'}}>{project.github_repo}</div>
                          </div>
                          <span className="material-symbols-outlined ml-auto" style={{fontSize:'16px', color:'#71717a'}}>open_in_new</span>
                        </a>
                      )}
                      <Link href={`/chat`}
                        className="flex items-center gap-3 p-3 rounded-md hover:bg-[#18181b] transition-all"
                        ...
                        <span className="material-symbols-outlined ml-auto" style={{fontSize:'16px', color:'#71717a'}}>arrow_forward</span>
                      </Link>
                    </div>
                  ) : (
            ...
            <div className="space-y-4">
              ...
              {/* Creator card */}
              <div className="rounded-md p-5 glass-card"
                style={{border:'1px solid rgba(39,39,42,0.15)'}}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-md overflow-hidden flex items-center justify-center"
                    ...
                    {project.owner?.avatar_url ? (
                      ...
                      <span className="material-symbols-outlined" style={{fontSize:'20px', color:'#10b981'}}>person</span>
                    )}
                  </div>
                  <div>
                ...
                {session && (
                  <Link href={`/profile/${project.owner_id}`}>
                    <button className="w-full mt-3 py-2 rounded-sm text-xs transition-all hover:bg-[#18181b]"
                      style={{border:'1px solid rgba(39,39,42,0.2)', fontFamily:'DM Mono', color:'#d4d4d8'}}>
                      View Full Profile
                    </button>
                  </Link>
                )}
              ...
              <div className="rounded-md p-5 glass-card"
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span style={{fontFamily:'DM Mono', fontSize:'11px', color:'#f4f4f5'}}>{spotsLeft} / {project.slots} open</span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{fontFamily:'DM Mono', fontSize:'11px', color:'rgba(194,198,214,0.5)'}}>Status</span>
                    <span style={{fontFamily:'DM Mono', fontSize:'11px', color:'#10b981', textTransform:'capitalize'}}>{project.status}</span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{fontFamily:'DM Mono', fontSize:'11px', color:'rgba(194,198,214,0.5)'}}>Posted</span>
                    <span style={{fontFamily:'DM Mono', fontSize:'11px', color:'#f4f4f5'}}>
